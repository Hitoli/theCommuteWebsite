...
fun ServiceSections() {
    ...
    Column(
        modifier = Modifier.id(Sections.Services.id)
            ...
            .border(width = 0.5.px, color = Theme.HeroColor.rgb, style = LineStyle.Solid).borderRadius(20.px)
...
fun ServicContent(services: Services, breakpoint: Breakpoint) {
    Column(
        modifier = Modifier.maxWidth(if (breakpoint > Breakpoint.MD) 700.px else 800.px).margin(20.px).padding(20.px)
    ) {
        Box(
            ...
            Image(
                modifier = Modifier.size(if (breakpoint >= Breakpoint.MD) 40.px else 20.px),
                src = services.image,
